...
export class UserDashboardComponent implements OnInit, OnDestroy {
  ...
  // ── Vacaciones ─────────────────────────────────────────────
  showVacacionesModal = false;
  vacacionesTexto = '';
  enviandoVacaciones = false;
  ...
  async enviarVacaciones() {
    if (!this.vacacionesTexto.trim() || this.enviandoVacaciones) return;
    this.enviandoVacaciones = true;
    try {
      const taskData = createEmptyTask({
        folioId: 'VACACIONES-' + Date.now(),
        cliente: 'Solicitud de Vacaciones',
        direccion: '',
        telefono: '',
        descripcion: this.vacacionesTexto,
        prioridad: 1,
        asignadoA: this.currentUser?.uid ? [this.currentUser.uid] : [],
        asignadoNombre: this.currentUser?.nombre ? [this.currentUser.nombre] : [],
        estado: 'pendiente',
      ...
      await this.tasksService.createTask(taskData);
      this.vacacionesTexto = '';
      this.showVacacionesModal = false;
      alert('Solicitud enviada correctamente');
    } catch (e) {
      console.error('Error al enviar solicitud de vacaciones:', e);
      alert('Error al enviar la solicitud');
    }
    this.enviandoVacaciones = false;
    this.cdr.detectChanges();
  }
  ...
  get progresoHoy(): number {
    if (this.tareasHoy.length === 0) return 0;
    return Math.round((this.completadasHoy / this.tareasHoy.length) * 100);
  }
