...
export class AdminDashboardComponent implements OnInit, OnDestroy {
  ...
  groupFilter = new Map<string, 'hoy' | 'proximas' | 'completadas' | 'vencidas'>();
  ...
  isGroupExpanded(uid: string): boolean {
    return this.expandedGroups.has(uid);
  }

  getGroupFilter(uid: string): 'hoy' | 'proximas' | 'completadas' | 'vencidas' {
    return this.groupFilter.get(uid) ?? 'hoy';
  }

  setGroupFilter(uid: string, filter: 'hoy' | 'proximas' | 'completadas' | 'vencidas', event: Event): void {
    event.stopPropagation();
    this.groupFilter.set(uid, filter);
  }

  getVencidasParaTecnico(uid: string, tareas: Task[]): number {
    const today = new Date(); today.setHours(0, 0, 0, 0);
    return tareas.filter(t => {
      if (t.estado === 'completada' || t.cliente?.includes('Pedido')) return false;
      const f = this.normalizeDate(t.fechaProgramada); f.setHours(0, 0, 0, 0);
      return f < today;
    }).length;
  }

  private isToday(fecha: any): boolean {
    const today = new Date(); today.setHours(0, 0, 0, 0);
  ...
  getTareasGrupo(uid: string, tareas: Task[]): Task[] {
    const filter = this.getGroupFilter(uid);
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const normales = tareas.filter(t => !t.cliente?.includes('Pedido'));
    switch (filter) {
      case 'hoy':
        return normales.filter(t => this.isToday(t.fechaProgramada) && t.estado !== 'completada');
      case 'proximas':
        return normales.filter(t => this.isFuture(t.fechaProgramada) && t.estado !== 'completada');
      case 'completadas':
        return normales.filter(t => t.estado === 'completada');
      case 'vencidas': {
        return normales.filter(t => {
  ...
  form = {
    cliente: '', direccion: '', telefono: '', descripcion: '', descripcionIngenieria: '',
  ...
  ngOnInit() {
    ...
    // Refresca el cronómetro de las tarjetas de técnico cada segundo (reloj continuo)
    ...
    this.usersSub = this.usersService.getAllUsers().subscribe(u => {
      this.users = u.filter(user => user.nombre && user.email && !user.suspendido);
      this.cdr.detectChanges();
    });
  ...
  // ── Asistencia ─────────────────────────────────────────────
  getAttendanceEst(uid: string): Attendance | undefined {
    return this.attendance.find(a => a.uid === uid);
  }

  getEstadoAtt(uid: string): string {
    const att = this.getAttendanceEst(uid);
    if (!att?.entrada) return 'sin_entrada';
    if (att.salida) return 'offline';
    return att.estado || 'disponible';
  }

  formatAttTime(date: any): string {
    if (!date) return '--:--';
    const d = date instanceof Date ? date : new Date(date);
    return d.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' });
  }

  // ── Datos en tiempo real por técnico (equivalente a operation-dashboard) ──
  ...
  getEstadoLabel(uid: string): string {
    const labels: Record<string, string> = {
      'offline': 'Offline', 'disponible': 'En traslado',
      'en_sitio': 'En sitio', 'traslado': 'En traslado', 'sin_entrada': 'Sin entrada'
    };
    return labels[this.getEstadoTecnico(uid)] ?? 'Offline';
  }

  isEnTraslado(uid: string): boolean {
    const e = this.getEstadoTecnico(uid);
    return e === 'traslado' || e === 'disponible';
  }

  getTareaActivaUid(uid: string): Task | null {
    return this.tasks.find(t => {
      const asignado = Array.isArray(t.asignadoA) ? t.asignadoA.includes(uid) : t.asignadoA === uid;
      return asignado && t.tiempoInicio != null;
    }) || null;
  }

  getClienteActivo(uid: string): string {
    return this.getTareaActivaUid(uid)?.cliente || '—';
  }

  getCronometroActivo(uid: string): string {
    const tarea = this.getTareaActivaUid(uid);
    if (!tarea) return '00:00:00';
    const inicio = (tarea.tiempoInicio as any)?.toDate?.()
      ?? (tarea.tiempoInicio instanceof Date ? tarea.tiempoInicio : new Date(tarea.tiempoInicio as any));
    const acumulado = (tarea.tiempoTotal || 0) * 60;
    const totalSeg = acumulado + Math.floor((new Date().getTime() - inicio.getTime()) / 1000);
  ...
  get technicianSummary() {
    ...
    return this.users.map(user => {
      ...
      // Progreso siempre por conteo de tareas: completadas / total del día
      const progreso = tareasHoy.length > 0
        ? Math.round((completadasHoy.length / tareasHoy.length) * 100)
        : 0;
  ...
  openModal(task?: Task) {
    ...
    if (task) {
      this.form = {
        ...
        descripcionIngenieria: task.descripcionIngenieria || '',
 ...
 async saveTask() {
  try {
    ...
    const taskData: any = {
      ...
      descripcionIngenieria: this.form.descripcionIngenieria || '',
  ...
  /** Suma el dinero efectivamente cobrado en las tareas asignadas al técnico */
  getPagosRecibidos(uid: string): number {
    return this.tasks
      .filter(t => t.asignadoA?.includes(uid))
      .reduce((sum, t) => {
        if (t.pago === 'pagado') {
          return sum + (t.monto || 0);
        } else if (t.pago === 'abono') {
          return sum + (t.montoAbonado || 0) + (t.anticipoRecibido || 0);
        }
        return sum;
      }, 0);
  }
  ...
  get tareasNormalesPorTecnico(): { uid: string; nombre: string; tareas: Task[] }[] {
    // Usa todas las tareas (sin filtro global) para que los filtros por grupo sean independientes
    const normales = this.tasks.filter(t => !t.cliente?.includes('Pedido'));
  ...
  getTareasParaTecnico(uid: string): Task[] {
    return this.tareasNormalesPorTecnico.find(g => g.uid === uid)?.tareas || [];
  }
